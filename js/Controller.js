class Controller {
    ...
    handleInput = async ({ target }) => {
        if (this.loadCommentsEl) {
            this.loadCommentsEl.removeEventListener(
                'click',
                this.loadCommentHandler
            );
        ...
        try {
            ...
            this.loadCommentsEl = document.querySelector(
                this.loadCommentsSelector
            );
            this.loadCommentsEl.addEventListener(
                'click',
                this.loadCommentHandler,
                { once: true }
            );
    ...
    loadCommentHandler = async ({ target }) => {
        try {
            const data = await this.#model.getCommentsByPostID(
                this.currentPost.id
            );
